...
- **Dual-Key Stealth Address Protocol (DKSAP)**: Academic foundation for stealth addresses, originally proposed for Bitcoin. We implement a simplified single-key variant suitable for Starknet's architecture.
...
| Term | Definition |
|------|------------|
| **Meta-address** | A public key pair (K) that recipients publish to receive stealth payments |
| **Ephemeral key** | A one-time key pair (r, R) generated by sender for each payment |
| **Shared secret** | The ECDH shared point S = r*K = k*R |
...
The protocol implements a simplified Dual-Key Stealth Address Protocol (DKSAP) on the STARK curve.
...
Recipient generates spending key pair:
```
k ← random ∈ [1, n-1]
K = k * G
```

Recipient publishes K as their meta-address via `register_stealth_meta_address(K.x, K.y)`.

**2. Sender Generates Stealth Address**

Sender fetches recipient's meta-address K, then:
```
r ← random ∈ [1, n-1]        // ephemeral private key
R = r * G                      // ephemeral public key
S = r * K                      // shared secret (ECDH)
...
```
announce(scheme_id=0, R.x, R.y, stealth_address, view_tag, metadata)
```
...
trait IStealthRegistry<TContractState> {
    ...
    fn register_stealth_meta_address(
        ...
        spending_key_x: felt252,
        spending_key_y: felt252
    );
...
|-----------|-------------|
| 0 | STARK curve ECDH with Poseidon hash |

Future schemes may be added for different curves or hash functions.
...
This specification uses a single spending key for simplicity. A dual-key scheme (separate spending and viewing keys) could be added in a future SNIP to support:
- Delegated scanning (give viewing key to a service)
- Watch-only wallets
...
To mitigate announcement spam/DoS, registries MAY implement **per-caller rate limits** (e.g., minimum block gap between announcements). This is optional and should be configurable (default disabled) to preserve open access.
...
|------|------------|------------|
| **Transaction amounts** | Fully visible | Integrate with Tongo or similar |
...
For stronger privacy, integrate with amount-hiding protocols like [Tongo](https://github.com/tongonetwork/tongo). See the "Future Work: Complete Privacy Stack" section for the integration roadmap.

## Reference Implementation

A complete reference implementation is available at: [https://github.com/omarespejel/starknet-stealth-addresses](https://github.com/omarespejel/starknet-stealth-addresses)

Components:
- Cairo contracts: `src/contracts/`
- TypeScript SDK: `sdk/`
...
|----------|-------|-------------|
| Unit tests | 22 | Registry, Account, Factory, View Tag |
| Security tests | 21 | Zero rejection, isolation, boundaries |
| Integration tests | 5 | Cross-component flows |
| E2E tests | 3 | Complete user workflows |
| **Fuzz tests** | 11 | Random inputs (100+ runs each) |
| **Invariant tests** | 9 | Properties that must always hold |
| **Gas benchmarks** | 8 | Performance baselines |
| **Stress tests** | 8 | High-load scenarios (50+ users, 100+ ops) |
...
| Contract | Address |
|----------|---------|
| StealthRegistry | `0x0638f00436e34e4d932b2f173eabcfb20e9173585ae5862bc1778fb645e0991c` |
| StealthAccountFactory | `0x06a715a0a2147db921bb25f4ed880cc4dba2a434851b8b32e6b1ca9ac31aa7cb` |
...
[Tongo](https://github.com/tongonetwork/tongo) is a privacy protocol for Starknet that hides transaction amounts using cryptographic commitments. Combining stealth addresses with Tongo creates a **complete privacy solution**:

| Privacy Property | Stealth Addresses | Tongo | Combined |
|------------------|-------------------|-------|----------|
| Recipient unlinkability | Yes | No | Yes |
| Amount hiding | No | Yes | Yes |
| Sender privacy | Partial* | Yes | Yes |

*Sender privacy is partial because the sender's address is visible in the `announce` transaction, but this can be mitigated with relayers.

### Comparison with Other Privacy Solutions

With stealth addresses + Tongo, Starknet can achieve strong privacy properties:

| Feature | Aztec | Tornado Cash | Starknet + Stealth + Tongo |
|---------|-------|--------------|---------------------------|
| Recipient privacy | Yes | Yes (pool-based) | Yes (stealth addresses) |
| Amount privacy | Yes | Fixed denominations | Yes (Tongo) |
| Programmable privacy | Yes | No | Yes (Cairo contracts) |
| Proof system | Custom ZK | Groth16 | STARK proofs |

**Key Differences from Aztec:**

- **Execution model**: Aztec uses a UTXO-like "notes" model optimized for privacy. Starknet uses account abstraction. Both have trade-offs.
- **Composability**: Stealth accounts implement SNIP-6, allowing interaction with existing Starknet protocols. However, private state transitions require careful design in both systems.
- **Privacy scope**: Aztec provides privacy natively at the protocol level. Starknet's approach is modular - stealth addresses and Tongo are opt-in layers.

**Advantages of the modular approach:**

- **Incremental adoption**: Users can adopt stealth addresses without full protocol changes
- **SNIP-6 compatibility**: Stealth accounts work with existing wallets and dApps that support standard accounts
- **Flexibility**: Different privacy levels for different use cases

### Technical Integration Pattern

The following pattern enables full transaction privacy by combining stealth addresses with Tongo:
...
│  SETUP (one-time):                                                │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │ Alice generates spending keypair (k, K = k*G)                │ │
│  │ Alice calls registry.register_stealth_meta_address(K.x, K.y) │ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                                                   │
│  PAYMENT (per transaction):                                       │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │ 1. Bob fetches Alice's meta-address K from registry          │ │
│  │                                                               │ │
│  │ 2. Bob generates ephemeral key r, computes:                   │ │
│  │    - R = r*G (ephemeral public key)                           │ │
│  │    - S = r*K (shared secret via ECDH)                         │ │
│  │    - P = K + hash(S)*G (stealth public key)                   │ │
│  │    - stealth_address = factory.compute_stealth_address(P)     │ │
│  │                                                               │ │
│  │ 3. Bob deposits funds into Tongo shielded pool                │ │
│  │    → Bob's funds join anonymity set                           │ │
│  │    → Amount becomes hidden                                    │ │
│  │                                                               │ │
│  │ 4. Bob transfers FROM Tongo TO stealth_address                │ │
│  │    → Sender hidden (Tongo anonymity set)                      │ │
│  │    → Amount hidden (Tongo commitments)                        │ │
│  │    → Recipient hidden (stealth address)                       │ │
│  │                                                               │ │
│  │ 5. Bob calls registry.announce(R, stealth_address, view_tag)  │ │
...
│  │    - Compute S' = k*R for matching announcements              │ │
│  │                                                               │ │
...
| Observe amounts | Tongo | Cryptographic commitments |
| Trace sender | Tongo + Relayer | Anonymity set + relayed announce |
| Timing analysis | Batching | Aggregate multiple operations |
| Graph analysis | Full integration | No on-chain link between parties |
...
- **Tongo**: Amount privacy protocol for Starknet (complementary to this SNIP)
